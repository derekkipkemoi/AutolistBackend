...
const mpesaApi = new Mpesa({
    ...
    lipaNaMpesaShortPass: process.env.LIPANAMPESASHORTPASS,
...
module.exports = {
    ...
    newUser: async (req, res, next) => {
        ...
        if (foundUser) {
            const message = {
                message:
                    "User with " + req.value.body.email + " already exists!",
            };
        ...
        if (process.env.NODE_ENV === "production") {
            const html = pug.renderFile(
                path.join(__dirname, "../views", "register.pug"),
                {
                    secretToken: secretString,
                }
            );
    ...
    contactUs: async (req, res, next) => {
        ...
        message =
            "Your message has been received successfully by info@autolist.co.ke. We will respond to your in shorwhile. Keep Checking your email " +
            email;
        res.status(200).json({ message });
    },
